package com.nx.forum_project.web.controller.rest;

import com.nx.forum_project.data.entity.Message;
import com.nx.forum_project.web.service.interf.CRUDService;
import com.nx.forum_project.web.service.interf.MessageService;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.beans.factory.annotation.Qualifier;
import org.springframework.web.bind.annotation.RequestMapping;
import org.springframework.web.bind.annotation.RestController;

@RestController
@RequestMapping(CRUDRestController.MESSAGE_URL)
public class MessageRestController extends CRUDRestController<Message, Long> {

    @Autowired
    @Qualifier("MessageServiceSpringDataImpl")
    private MessageService messageService;

    @Override
    CRUDService<Message, Long> getService() {
        return messageService;
    }
}
